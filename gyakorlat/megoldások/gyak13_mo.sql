...
select mkolcsonzes.*
, count(*) over (partition by film) hanyszor
, sum(napok) over (partition by film) hanynap
from mkolcsonzes;
...
select mkolcsonzes.*
, round(avg(napok) over (partition by kolcsonzo),2) atlagnap
from mkolcsonzes;

select a.*
, case when napok>atlagnap then 'átlagosnál hosszabb' end eltérés
from
(select mkolcsonzes.*
, round(avg(napok) over (partition by kolcsonzo),2) atlagnap
from mkolcsonzes) a
...
select mkolcsonzes.*
, round(avg(napok) over (order by sorszam rows between 4 preceding and current row),2) atlagnap
from mkolcsonzes;
...
select mkolcsonzes.*
, sum(napok) over 
(partition by kolcsonzo 
order by sorszam 
rows between unbounded preceding and current row) halmozott_nap
from mkolcsonzes;

/*5. Írjuk ki az mdvd táblából a 10 legmagasabb pontszámú filmet! 
(Vigyázzanak a null értékekkel!)*/

select mdvd.*, rank() over (order by imdb_pontszam desc nulls last) rang
from mdvd;
...
select kid, nev,hányszor--, round(avg(hányszor) over (partition by nem),2) nem_átlag
...
/*7. Volt-e olyan eset, amikor kétszer egymás után ugyanazt a filmet vitték
ki! Listázzuk ki a két egymás utáni kölcsönzés sorszámát és a film címét!*/

select elozo_szam, sorszam, film
from
(select mkolcsonzes.*
, lag(sorszam) over (order by sorszam) elozo_szam
, lag(film) over (order by sorszam) elozo
from mkolcsonzes)
where film=elozo
;
...
select kolcsonzo,nev,elozo_szam, sorszam, film
from
(select mkolcsonzes.*
, lag(sorszam) over (partition by kolcsonzo order by sorszam) elozo_szam
, lag(film) over (partition by kolcsonzo order by sorszam) elozo
,nev
from mkolcsonzes join mszemely on kolcsonzo=kid)
where film=elozo
;

/*9. Osszuk a filmeket a keletkezés éve alapján 10 egyenlõ csoportra!*/

select mdvd.*, ntile(10) over (order by ev) kortized
from mdvd;
