...
SELECT id, vnev, knev, felv_nap FROM dolgozo
where to_char(felv_nap,'DD')='01';

/*8. Írassuk ki a dolgozók azonosítóját, nevét és a születésnapjukat kiírt hónapnévvel!*/
SELECT ID, vnev, knev, to_char(szuldat,'monthdd.') születésnap
FROM dolgozo;
...
SELECT ID, vnev, knev, to_char(szuldat,'monthdd.') születésnap
FROM dolgozo
ORDER BY to_char(szuldat,'mm.dd');

SELECT ID, vnev, knev, to_char(szuldat,'monthdd.') születésnap
FROM dolgozo
ORDER BY to_date(születésnap,'monthdd.');

/*10. Írassuk ki a dolgozókról, hogy milyen napon születtek (hétfõ, kedd stb.)*/

SELECT ID, vnev, knev, to_char(szuldat,'DAY') születésnap_napja
FROM dolgozo;

/*11. Írassák ki, hogy a saját születésnapjuk milyen napra esett!*/

SELECT to_char(to_date('1980-09-23','YYYY-MM-DD'),'DAY') születésnap_napja FROM dual;

/*12. Írassuk ki, hogy az egyes dolgozók hány napja dolgoznak a cégnél!
A dátumot idõ nélkül a TRUNC függvénnyel kapjuk meg.*/

SELECT ID, vnev, knev, trunc(SYSDATE)-felv_nap hány_napja
FROM dolgozo;
...
SELECT * FROM dolgozo
WHERE
szuldat<to_date('80-01','RR-MM')
AND
felv_nap>=to_date('11-01','RR-MM')
ORDER BY szuldat;
...
SELECT * FROM dolgozo
WHERE felv_nap-szuldat<20*365.25;
